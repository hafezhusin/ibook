...
<head>
    ...
    <title>404 - Halaman Tidak Dijumpai | {{ $tetapan['nama_sistem'] ?? 'iBook 2.0' }}</title>
...
<div class="text-center p-10 max-w-md">
    <div class="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-6" style="background:#1a1a2e">
        <span class="text-4xl font-black" style="color:#f59e0b">404</span>
    </div>
    <h1 class="text-2xl font-bold text-gray-800 mb-2">Halaman Tidak Dijumpai</h1>
    <p class="text-gray-500 mb-6">Halaman yang anda cari tidak wujud atau telah dialihkan.</p>
    <a href="{{ auth()->check() ? route('dashboard') : route('login') }}"
       class="inline-block px-6 py-3 rounded-lg font-semibold text-white"
       style="background:#f59e0b">
        &larr; Kembali ke {{ auth()->check() ? 'Dashboard' : 'Log Masuk' }}
    </a>
</div>
</body>
</html>
